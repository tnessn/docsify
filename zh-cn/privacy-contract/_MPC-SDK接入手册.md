...
- [MPC SDK接入手册](#MPC-SDK%e6%8e%a5%e5%85%a5%e6%89%8b%e5%86%8c)
...
MPC计算虚拟机`MPC VM`作为 PlatON 计算架构中作为关键组件，提供了MPC计算任务的动态执行环境。
...
* 数据提供方：提供数据的客户端，目前只有两方，Alice方和Bob方。
* 计算发起方：发起计算请求的一方，可以是数据提供方，也可以是数据提供方之外的第三方。
...
此环节将会向读者描述如何构建隐私数据服务接入，一个代表Alice, 一个代表Bob。以下步骤将以最简方式演示
...
`Callback.Client.Endpoints`为Client所在的服务器地址。
